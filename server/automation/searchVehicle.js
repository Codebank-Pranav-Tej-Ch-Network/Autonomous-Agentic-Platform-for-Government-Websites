...
async function searchVehicle(data) {
    ...
    try {
        ...
        if (!captcha) {
            ...
            await page.goto('http://localhost:5000/vahan', {
...
await page.waitForTimeout(2000); // Wait 2 seconds for captcha to render
...
const captchaBuffer = fs.readFileSync(screenshotPath);
const captchaImageBase64 = `data:image/png;base64,${captchaBuffer.toString('base64')}`;

console.log('✅ Captcha screenshot saved');
console.log('📸 Captcha base64 length:', captchaImageBase64.length);

return {
    success: true,
    needsCaptcha: true,
    captchaImage: screenshotPath,
    captchaImageBase64: captchaImageBase64,  // ← Add this line
...
// **ADD THIS: Handle alerts before clicking submit**
...
// Check if resultCard is visible
const resultVisible = await page.locator('#resultCard').isVisible().catch(() => false);
console.log('📊 Result card visible:', resultVisible);

if (!resultVisible) {
    // Take screenshot for debugging
    const debugPath = path.join(__dirname, `../temp/vahan-error-${Date.now()}.png`);
    ...
    return {
        success: false,
        message: 'Results not showing - captcha might be incorrect or vehicle not found'
    };
}
