...
class ITRFilingAutomation extends EventEmitter {
  ...
  async login() {
    try {
      // STEP 1: Click the "Login" link in the header to navigate to login page
      // Our homepage has a link with href="/login" in the top-right corner
      await this.page.click('a[href="/login"]');
      logger.info('Clicked login link in header');
      
      // STEP 2: Wait for the login page to fully load
      // We verify this by checking for the login form heading
      ...
      // The input has name="pan" attribute
      ...
      // For our test portal, we use the hardcoded test password
      await this.page.fill('input[name="password"]', 'TestPassword123');
      logger.info('Filled password');
      
      // STEP 5: Fill in the CAPTCHA field
      // Our mock portal accepts "MOCK" as a valid CAPTCHA for testing
      ...
      // The submit button has type="submit" and text "Login to Continue"
      await this.page.click('button[type="submit"]');
      logger.info('Clicked login submit button');
      
      // STEP 7: Wait for successful login
      // After login, we're redirected to the dashboard which has a welcome card
